...
Example: `-i relp --port 514`
Example: `-i relp -p 10514`
...
- `rsp` - Response from the server acknowledging receipt
...
Write events to an rsyslog server using the RELP (Reliable Event Logging Protocol).

RELP is a reliable protocol designed for log transmission that ensures message delivery and maintains message ordering. It's commonly used with rsyslog servers.

Parameters: `-o relp --hostname {hostname or ip} --port {port number}`

Example: `-o relp --hostname localhost --port 514`
Example: `-o relp -hn syslog.example.com -p 514`

The RELP output item will:

- Establish a connection to the specified rsyslog server
- Send events using the RELP protocol
- Handle acknowledgments from the server
- Automatically retry on connection failures

To test the RELP output, you can start an rsyslog server listening on a port and send events to it:

`java -jar LogGenerator-{version}.jar -i file --name src/test/data/test.txt -o relp --hostname localhost --port 514`

Or combine with other modules to send syslog-formatted events:

`java -jar LogGenerator-{version}.jar -i counter --string "Test event:" -f header --string "<{pri:}>{date:MMM dd HH:mm:ss} localhost LogGenerator[{random:1-65535}]: " -o relp --hostname localhost --port 514 --limit 100`
